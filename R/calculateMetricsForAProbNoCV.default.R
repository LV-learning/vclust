calculateMetricsForAProbNoCV <- function(validator,
                                         pseudo_a_prob,
                                         r_pseudo,
  ...
  UseMethod("calculateMetricsForAProbNoCV")
}
#' @importFrom  stats aggregate var
calculateMetricsForAProbNoCV.default <- function(validator,
                                                 pseudo_a_prob,
                                                 r_pseudo,
                                                 input_and_pp_and_var_df,
                                                 seed_num,
  ...
  for(j in 1:r_pseudo){
    if (supervised_model %in% c("logistic regression", "logistic")) {
      metrics_ij <- genLogisticMetricsOpt(y = pseudo_a_prob[,j],
      ...
      metrics_ij <- genMetricsABinaryXOpt(y = pseudo_a_prob[,j],
                                          predictor_part = predictor_part,
                                          input_and_pp_and_var_df = input_and_pp_and_var_df,
                                          lr_maxiter = lr_maxiter)
    }
