dichPseudoByPathAModel <- function(pp_dt,
                                   ##model classes
                                   n,
                                   r_pseudo,
                                   K_fold,
                                   ...
                                   if_CV) {
  print(
    '########################using dichPseudoByPathAModel##############################'
  )
  ##extract names from PCD
  PCD_list_name <- names(PCD_list)
  print("PCD_list_name is")
  print(PCD_list_name)
  ...
  for (v_id in 1:length(validators)) {
    ...
    if (validation_data_fraction != 1) {
      ...
      ##Don't forgot check if train_ind or test_ind is null
