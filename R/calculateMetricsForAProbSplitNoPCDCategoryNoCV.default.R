calculateMetricsForAProbSplitNoPCDCategoryNoCV <- function(validator,
                                                           categoryVec,
                                                           ...
                                                           n,
                                                           pcd_dropping_pct){
  UseMethod("calculateMetricsForAProbSplitNoPCDCategoryNoCV")
}
#' @importFrom  stats aggregate var
calculateMetricsForAProbSplitNoPCDCategoryNoCV.default <- function(validator,
                                                                   categoryVec,
                                                                   input_and_pp_and_var_df,
                                                                   seed_num,
                                                                   validation_data_fraction,
  ...
  if (supervised_model %in% c("logistic regression", "logistic")){
    metrics_ij <- genLogisticMetricsOpt(y = cat_vec_train,
    ...
    metrics_ij <- genMetricsABinaryXOpt(y = cat_vec_train,
                                        predictor_part = predictor_part,
                                        input_and_pp_and_var_df = input_and_pp_and_var_df[train_ind,],
                                        lr_maxiter = lr_maxiter)
  }
  ...
  ##results without CV
  if (supervised_model %in% c("logistic regression", "logistic")){
    metrics_j <- genLogisticMetricsNoCV(y = cat_vec_train,
                                        predictor_part = predictor_part,
                                        input_and_pp_and_var_df = input_and_pp_and_var_df[train_ind,],
                                        y_test = cat_vec_test,
    ...
    metrics_j <- genMetricsABinaryXNoCV(y = cat_vec_train,
                                        predictor_part = predictor_part,
                                        input_and_pp_and_var_df = input_and_pp_and_var_df[train_ind,],
                                        y_test = cat_vec_test,
                                        input_and_pp_and_var_df_test =
