...
calculateMetricsForAProbNoPCDCategoryNoCV <- function(validator,
                                                      categoryVec,
                                                      ...
                                                      n,
                                                      pcd_dropping_pct){
  UseMethod("calculateMetricsForAProbNoPCDCategoryNoCV")
}
#' @importFrom  stats aggregate var
calculateMetricsForAProbNoPCDCategoryNoCV.default <- function(validator,
                                                              categoryVec,
                                                              input_and_pp_and_var_df,
                                                              seed_num,
                                                              lr_maxiter,
  ...
  if (supervised_model %in% c("logistic regression", "logistic")){
    metrics_ij <- genLogisticMetricsOpt(y = categoryVec,
    ...
    metrics_ij <- genMetricsABinaryXOpt(y = categoryVec,
                                        predictor_part = predictor_part,
                                        input_and_pp_and_var_df = input_and_pp_and_var_df,
                                        lr_maxiter = lr_maxiter)
  }
